...
@router.get("/export/combined-pdf")
async def export_combined_attendance_pdf(
    start_date: Optional[str] = Query(None, description="Start date (YYYY-MM-DD)"),
    end_date: Optional[str] = Query(None, description="End date (YYYY-MM-DD)"),
    current_teacher: dict = Depends(get_current_teacher),
):
    """Export combined attendance report for all subjects as a professional PDF document.

    Generates aggregated statistics for verified students across all subjects managed by the teacher.
    """
    try:
        teacher_id = current_teacher["id"]
        ...
        summary_data = [["Subject", "Code", "Students", "Avg Attendance"]]
        ...
        # --- Loop through subjects and generate report sections ---
        for index, subject in enumerate(subjects):
            # No PageBreak needed at start because we just added one after summary
            # But we need PageBreak between subjects
            if index > 0:
                elements.append(PageBreak())
